...
then
    ...
    mkdir -p "${STCV_BUCKET_1_MNT_DIR}" && chmod 777 ${STCV_BUCKET_1_MNT_DIR}
    s3fs ${STCV_BUCKET_1_NAME} ${STCV_BUCKET_1_MNT_DIR} -o allow_other -o uid=0,gid=0,endpoint=eu-central-1,iam_role=auto
    ln -s ${STCV_BUCKET_1_MNT_DIR} /mnt/bucket
else
...
then
    ...
    mkdir -p "{$STCV_BUCKET_2_MNT_DIR}" && chmod 777 ${STCV_BUCKET_2_MNT_DIR}
    s3fs ${STCV_BUCKET_2_NAME} ${STCV_BUCKET_2_MNT_DIR} -o allow_other -o uid=0,gid=0,endpoint=eu-central-1,iam_role=auto
    ln -s ${STCV_BUCKET_2_MNT_DIR} /mnt/bucket_secondary
else
...
mkdir -p ${STCV_MONGODB_DATA_DIR} && chmod 777 ${STCV_MONGODB_DATA_DIR}
mongod --fork --dbpath ${STCV_MONGODB_DATA_DIR} --bind_ip_all --logpath /var/log/mongodb/mongod.log --port ${STCV_MONGODB_PORT}
...
/usr/sbin/php-fpm8.1 --allow-to-run-as-root

#########

echo "Starting FiftyOne"
export FIFTYONE_DATABASE_URI=mongodb://127.0.0.1:${STCV_MONGODB_PORT}
mkdir -p ${STCV_FIFTYONE_DATA_DIR} && chmod 777 ${STCV_FIFTYONE_DATA_DIR}
...
mkdir -p ${STCV_LABELSTUDIO_DATA_DIR} && chmod 777 ${STCV_LABELSTUDIO_DATA_DIR}
label-studio start --username email@email.com --password password > /dev/null 2>&1 &

echo "Starting TensorBoard"
if [ ${STCV_TENSORBOARD_1_ENABLE} = "True" ]
then
    mkdir -p ${STCV_TENSORBOARD_1_DATA_DIR} && chmod 777 ${STCV_TENSORBOARD_1_DATA_DIR}
...
then
    mkdir -p ${STCV_TENSORBOARD_2_DATA_DIR} && chmod 777 ${STCV_TENSORBOARD_2_DATA_DIR}
...
then
    mkdir -p ${STCV_MLFLOW_1_DATA_DIR} && chmod 777 ${STCV_MLFLOW_1_DATA_DIR}
...
then
    mkdir -p ${STCV_MLFLOW_2_DATA_DIR} && chmod 777 ${STCV_MLFLOW_2_DATA_DIR}
